...
interface CourseServerPaginationDataTableProps<TData, TValue> {
...
export function CourseServerPaginationDataTable<TData, TValue>({
  columns,
  data,
  meta,
  searchKey,
...
}: CourseServerPaginationDataTableProps<TData, TValue>) {
  ...
  const { data: categories = [], isLoading: categoriesLoading } = useCategory<Category[]>("course");
  ...
  const selectedCategory = categories.find((cat: Category) => cat.id === currentCategory);
      ...
      <div className="flex w-full flex-col-reverse items-center justify-between gap-4 overflow-auto p-1 sm:flex-row sm:gap-8 mt-4">
        <div className="flex-1 whitespace-nowrap text-sm text-muted-foreground">
          ...
          {(selectedCategory || selectedLanguage) && (
            <span className="ml-2">
              • Filtered by: 
              {selectedCategory && <strong className="ml-1">{selectedCategory.name}</strong>}
              {selectedCategory && selectedLanguage && <span className="mx-1">•</span>}
              {selectedLanguage && <strong className="ml-1">{selectedLanguage.label}</strong>}
            </span>
          )}
        </div>
