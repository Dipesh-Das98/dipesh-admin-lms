...
import { createFamilyHealthCourse } from "@/actions/dashboard/familyHealth/create-family-health-course";
import { updateFamilyHealthCourse } from "@/actions/dashboard/familyHealth/update-family-health-course-by-id";
import { useQueryClient } from "@tanstack/react-query";

const familyHealthCourseSchema = z.object({
...
type CreateStoryFormValues = z.infer<typeof familyHealthCourseSchema>;
...
const MainForm: React.FC<MainFormProps> = ({ mode, initialValues }) => {
  ...
  const form = useForm<CreateStoryFormValues>({
    resolver: zodResolver(familyHealthCourseSchema),
    defaultValues: {
  ...
  const onSubmit = async (values: CreateStoryFormValues) => {
    try {
      ...
      if (mode === "create") {
        const response = await createFamilyHealthCourse(values);
        ...
        router.push(`/dashboard/familyHealth/edit/${createdId}`);
      } else if (mode === "edit" && values.id) {
        const { id, thumbnailUrl, videoUrl, ...rest } = values;

        const response = await updateFamilyHealthCourse({
          id,
          ...rest,
        ...
        router.push(`/dashboard/familyHealth`);
      }
