...
import {
  updateMovie,
  UpdateMovieData,
} from "@/actions/dashboard/movie/update-movie";
import {
  Form,
  FormControl,
  FormField,
...
export function BasicSettingsForm({
  ...
  const form = useForm<BasicSettingsFormData>({
    ...
    defaultValues: {
      ...
      backgroundColor: movie.backgroundColor || "#3b82f6",
      grade: movie.grade || "",
  ...
  const updateMutation = useMutation({
    mutationFn: (data: UpdateMovieData) => updateMovie(data),
    onSuccess: (response) => {
      if (response.success) {
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Update the core information for this movie
            </p>
          </div>
        </div>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="space-y-3">
                  ...
                  <FormItemInfo message="📚 A catchy title that captures the essence of your movie" />
                </FormItem>
              )}
            />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="space-y-3">
                  ...
                  <FormMessage className="text-xs" />
                  <FormItemInfo message="📝 A compelling description that helps users understand what the movie is about (max 500 characters)" />
                </FormItem>
              )}
            />
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      <FormControl>
                        <SelectTrigger className="h-12 border-border/50 bg-background/50 focus:bg-background transition-colors duration-200 focus:ring-2 focus:ring-primary/20">
                          <SelectValue placeholder="Select the grade level" />
                        </SelectTrigger>
                      </FormControl>
                ...
                render={({ field }) => (
                  <FormItem className="space-y-3">
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      <FormControl>
                        <SelectTrigger className="h-12 border-border/50 bg-background/50 focus:bg-background transition-colors duration-200 focus:ring-2 focus:ring-primary/20">
                          <SelectValue placeholder="Select the movie language" />
                        </SelectTrigger>
                      </FormControl>
                      <SelectContent className="border-border/50">
              ...
              render={({ field }) => (
                <FormItem className="space-y-3">
                  ...
                  >
                    <FormControl>
                      <SelectTrigger className="h-12 border-border/50 bg-background/50 focus:bg-background transition-colors duration-200 focus:ring-2 focus:ring-primary/20">
                        <SelectValue placeholder="Choose a category for the movie" />
                      </SelectTrigger>
                    </FormControl>
                    <SelectContent className="border-border/50">
                  ...
                  <FormMessage className="text-xs" />
                  <FormItemInfo message="🏷️ Categorizing helps organize and filter movies effectively" />
                </FormItem>
              )}
            />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="space-y-3">
                  ...
                  <FormMessage className="text-xs" />
                  <FormItemInfo message="🎨 The background color helps create visual distinction and mood for the movie" />
                </FormItem>
              )}
            />
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="space-y-3 flex flex-col justify-end h-full">
                    <div className="flex flex-row items-center justify-between">
                      <div className="space-y-0.5">
                        ...
                        <p className="text-xs text-muted-foreground">
                          Set the movie as active or inactive
                        </p>
                      </div>
                      <FormControl>
                        <Switch
                    ...
                    <FormMessage className="text-xs" />
                    <FormItemInfo message="🚦 Active movies are visible to users, inactive ones are hidden" />
                  </FormItem>
                )}
              />
            </div>
